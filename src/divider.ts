...
class Divider extends BlockEmbed {
    static create (options: Options) {
        ...
        if (options.text) {
            ...
            const lineLeft = `<span contenteditable="false" style="width: ${ width[0] };${ commonStyle }${ options.cssText || '' }" class="${ options.className || '' }" ></span>`
            const lineRight = `<span contenteditable="false" style="width: ${ width[1] };${ commonStyle }${ options.cssText || '' }" class="${ options.className || '' }" ></span>`
            parentNode.style.display = 'flex'
            parentNode.style.textAlign = 'center'
            parentNode.innerHTML = `${ lineLeft }<span style="display: inline-block;${options.text.childrenStyle}" contenteditable="true">${ options.text.children || 'placeholder' }</span>${ lineRight }`
        } else {
